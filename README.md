...
| `opsmaster` Actions | `default_workflow_permissions: read`, `can_approve_pull_request_reviews: false` | Set 2026-09-18 (`ginsys/opsmaster#661`). Every job in every workflow there already declares its own explicit `permissions:` key, so this changes nothing about how any of them actually run — it only removes an unused implicit-write fallback. **Not a security ceiling**: an explicit job-level `permissions:` block always takes effect exactly as written, regardless of this repo default — a `pull_request`-triggered job's own copy of that block can still request `write` for its own run. Verified against GitHub's docs before recording this: "Anyone with write access to a repository can modify the permissions granted to the `GITHUB_TOKEN` … by editing the `permissions` key in the workflow file." Two residual findings from that PR rest on this distinction and remain open pending a structural fix, not this setting |
